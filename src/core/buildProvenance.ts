...
export function logBuildProvenance(): void {
  ...
  if (provenance.environment !== 'production') {
    // This is deliberately limited to non-sensitive build identifiers and is only emitted in
    // non-production artifacts so staging devices can be compared to local source.
